  ...
  return React.createElement("div", {ref, style:{position:"relative"}},
    ...
    open && React.createElement("div", {
      ...
      localModels
        ...
        .length === 0
        ? React.createElement("div",{style:{padding:"5px 18px 4px",fontSize:"12px",color:W.fgDim}},
            "Start Jan.ai en zet de API-server aan (Settings → Local API Server)")
        : localModels.filter(m => m.startsWith("jan:")).map(m =>
            React.createElement("button", {
              key:m, onClick:()=>select(m),
              style:{
                display:"flex", alignItems:"center", gap:"8px",
